...
export default function MobileTable() {
  ...
  return (
    <div className='relative mb-10 space-y-1 lg:hidden'>
      <div className='flex h-8 items-center justify-between bg-gray-100 px-3 text-xs font-bold'>
        <div>{t('website')}</div>
        <div>{t('submission')}</div>
      </div>
      <div className='flex flex-col gap-1'>
        {STARTUP_LIST.map((item) => (
          <div key={item.DA} className='flex min-h-[99px] items-center justify-between bg-gray-100 p-3'>
